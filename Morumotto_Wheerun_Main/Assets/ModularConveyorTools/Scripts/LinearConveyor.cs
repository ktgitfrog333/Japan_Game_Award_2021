...
[RequireComponent(typeof(ConveyorMoveCharacter))]
public class LinearConveyor : MonoBehaviour {
    ...
    private ConveyorMoveCharacter _conveyorMoveCharacter;

	// Use this for initialization
	void Start () {
		RefreshReferences ();

        _conveyorMoveCharacter = GetComponent<ConveyorMoveCharacter>();

        if (0f < _conveyorMoveCharacter.Speed)
        {
            ChangeSpeed(_conveyorMoveCharacter.Speed);
        }
        else
        {
            ChangeSpeed(speed);
