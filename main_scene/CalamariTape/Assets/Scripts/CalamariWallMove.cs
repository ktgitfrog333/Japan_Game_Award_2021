...
public class CalamariWallMove : MonoBehaviour
{
    ...
    public bool _wallRunVertical {
        private set;
        get;
    } = false;

    /// <summary>壁走り（横）</summary>
    public bool _wallRunHorizontal {
        private set;
        get;
    } = false;

    /// <summary>RayCast判定の距離の処理内で扱う</summary>
    public float _registMaxDistance
    {
        private set;
        get;
    }
    ...
    {
        // 大きさに合わせてRayの距離を計算
        _registMaxDistance = CalamariStateConf.ParameterMatchScale(_state._maxDistance, _state._maxMaxDistance, _scaler.Scale);
